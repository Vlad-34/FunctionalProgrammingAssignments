...
import Parsec (Parser, andThen, orElse, pMap, pThen, number)
...
-- Success ("abc", "")
string :: Parser String -- use take and with
-- string = P.fail "TODO (string)"
string = (P.number `P.with` stringAux)

stringAux :: Int -> Parser String
stringAux nr = P.some (P.char ':') `P.pThen` P.take nr
...
dict = P.between (P.char 'd') (P.char 'e') (P.many keyValue)

keyValue :: Parser BencodeKW
keyValue = string `P.andThen` value
